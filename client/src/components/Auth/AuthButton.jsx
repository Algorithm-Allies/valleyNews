  ...
  return (
    ...
    >
      <div className="relative">
        <span
          className={`text-base ${
            navigation.state === "submitting" ? "text-white/60" : "text-white"
          }`}
        >
          {children}
        </span>
        <span className="absolute left-0 -translate-x-6  top-1/2 -translate-y-1/2 ">
          {navigation.state === "submitting" && (
            <Spinner className="size-4 text-custom-orange animate-spin" />
          )}
        </span>
      </div>
    </button>
